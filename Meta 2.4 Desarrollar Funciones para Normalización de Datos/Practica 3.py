"""Realizar una función que normalice los datos usando escalado simple que reciba como parámetro un DataFrame 
...
def normalizacion_escalado_simple(data:pd.DataFrame, columnas:list):
    for i in data: # Ciclo en el dataframe para ubicar cada columna
        for j in columnas: # Ciclo en la lista de columnas recibida por medio del parametro
            if j == i: # Comparando la lista de columnas recibida con las del dataframe
                ...
                data[i] = data[i] / data_max # Calculando la normalizacion de los datos y actualizando la columna
    return data
...
if __name__ == "__main__":
    ...
    print(normalizacion_escalado_simple(data, columnas))